...
export default function Admin() {
  ...
  const [reminderState, setReminderState] = useState('idle') // idle | sending | done | error
  ...
  async function sendReminder() {
    ...
    const { data, error } = await supabase.functions.invoke('send-pick-reminder', { body: {} })
    if (error) {
      setReminderState('error')
      setReminderResult(error.message ?? 'Send failed')
      return
    }
    if (data?.skipped) {
      setReminderState('done')
      setReminderResult(`Skipped: ${data.skipped}.`)
      return
    }
    setReminderState('done')
    setReminderResult(`Sent to ${data?.emailsSent ?? 0} people for Week ${data?.week}.`)
  }
  ...
  return (
    <div>
      <h1 className="font-display font-bold text-3xl text-chalk tracking-wide mb-6">ADMIN</h1>
      ...
      <div className="bg-panel border border-line rounded-md shadow-sm p-4 mb-8">
        <div className="font-mono text-xs uppercase text-chalkDim mb-2">Weekly recap email</div>
        <p className="text-chalkDim text-sm mb-3">
          Sends everyone their personal record, this week&apos;s results, and the top-10 standings.
        </p>
        <button
          onClick={sendRecap}
      ...
      <div className="bg-panel border border-line rounded-md shadow-sm p-4">
        <div className="font-mono text-xs uppercase text-chalkDim mb-2">Friday picks reminder</div>
        <p className="text-chalkDim text-sm mb-3">
          Runs automatically every Friday at 9am Central. Use this to test it or send an extra nudge —
          it targets whichever week has the next unplayed game, and lists each person&apos;s
          still-missing picks.
        </p>
        <button
          onClick={sendReminder}
          disabled={reminderState === 'sending'}
          className="bg-panelLight text-chalk border border-line font-mono uppercase text-sm font-bold px-5 py-2.5 rounded hover:border-amber transition disabled:opacity-50"
        >
          {reminderState === 'sending' ? 'Sending…' : 'Send reminder now'}
        </button>
        {reminderResult && (
          <p className={`text-sm font-mono mt-3 ${reminderState === 'error' ? 'text-crimson' : 'text-amber'}`}>
            {reminderResult}
          </p>
        )}
      </div>
